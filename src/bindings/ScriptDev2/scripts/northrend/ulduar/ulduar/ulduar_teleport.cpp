...
bool GossipHello_ulduar_teleporter(Player *player, GameObject *gobject)
{
    ScriptedInstance *pInstance = (ScriptedInstance *) gobject->GetInstanceData();
	...
	*/
    player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, gobject->GetGUID());
    return true;
}

bool GossipSelect_ulduar_teleporter(Player *player, GameObject *gobject, uint32 sender, uint32 action)
{
...
void AddSC_ulduar_teleport()
{
    ...
    newscript->pGOGossipHello = &GossipHello_ulduar_teleporter;
    newscript->pGOGossipSelect = &GossipSelect_ulduar_teleporter;
